Ext.define('SupermarketInvoicingSystem.view.indent.IndentViewController', {
  ...
  var placeType = 'WARE';
  var store = Ext.getCmp('rightList').getStore();
  Ext.apply(store.proxy.extraParams, {commodityType:selectedType, placeId:selectedWare, name:keyWord, searchType:searchType, placeType:placeType});
  store.load({params:{start:0, limit:20, page:1}});
  ...
  Ext.apply(store.proxy.extraParams, {placeId:toPlaceId, searchType:searchType, placeType:placeType});
  store.load({params:{start:0, limit:20, page:1}});
  ...
  for (var i in leftgridData) {
    leftgridDataJson.push({'name':leftgridData[i].get('name'), 'num':leftgridData[i].get('num'), 'cost':leftgridData[i].get('cost'), 'price':leftgridData[i].get('price')});
  }
  ...
  var leftgridDataJson = [];
  for (var i in leftgridData) {
    leftgridDataJson.push({'name':leftgridData[i].get('name'), 'num':leftgridData[i].get('amount'), 'cost':leftgridData[i].get('cost'), 'price':leftgridData[i].get('price')});
  }
  var removecharacter = Ext.encode(leftgridDataJson);
  Ext.getCmp('commoditiesJSON').setValue(removecharacter);
  var values = form.getValues();
